...
test('paintSquare respects protected cells', () => {
  const map = new TileMap({ width: 3, height: 3, tileSize: 1, defaultTileId: 0 });
  const patch = map.paintSquare(1, 1, 3, 2, (x, z) => x !== 1 || z !== 1);

  assert.equal(patch.indices.length, 8);
