...
function evaluateCandidate(candidate, candidateEvaluator) {
  ...
  for (const key of Object.keys(metadata)) {
    if (Object.hasOwn(candidate, key)) {
      throw new Error(`candidateEvaluator metadata cannot override candidate field: ${key}.`);
    }
  }
 ...
 * maxAccepted limits each owner chunk's authoritative candidate set before
 * spacing, so discarded candidates cannot create invisible blockers.
 */
export function buildStableChunkManifest({
  ...
  if (candidateEvaluator !== null && typeof candidateEvaluator !== 'function') {
    throw new Error('candidateEvaluator must be a function or null.');
  }
